...
//   double percErrorSum=0;
          fprintf(CPUoutputFile, "3 %d %d \n", NZ, NY);
	    fprintf(GPUoutputFile, "3 %d %d \n", NZ, NY);
  for (k=0; k<NZ; k++) {
    for (j=0; j<NY; j++) {
      for (i=0; i<NX; i++) {

        ind = i + j*NX + k*NX*NY;
	   fprintf(CPUoutputFile, "%f ",h_u1[ind]);
	    fprintf(GPUoutputFile, "%f ", h_u2[ind]);

        err += (h_u1[ind]-h_u2[ind])*(h_u1[ind]-h_u2[ind]);
	delta = fabs(h_u1[ind]-h_u2[ind]);
	double ref1  = fabs(h_u2[ind]);
	//percErrorSum += delta*100.0/ref1;
             if (delta > MIN_EPSILON_ERROR)
                   nDifferent++;
              bool include =true;
            double temp = percentDiffOurs(h_u1[ind], h_u2[ind],include);
                if(include) {
                   sumRelativeError += temp;
                   nIncluded++;
                }
                bool include2 =true;
                double temp2 = percentDiff_skipzero(h_u1[ind], h_u2[ind], include2);
                if(include2) {
                   sumRelativeError_skipzero += temp2;
                   nIncluded2++;
                }

                double temp3 = percentDiff_noskip(h_u1[ind], h_u2[ind]);
  ...
  //double finalPercError = percErrorSum/(NX*NY*NZ); // no need of *100
 // printf("ACT_percLossInQoR %E\n", finalPercError);
  printf("\n rms error = %f \n",sqrt(err/ (float)(NX*NY*NZ)));

        double finalPercError = (100.0*nDifferent)/(float)(NX*NY*NZ);
        //printf("ACT_percLossInQoR %E\n", finalPercError);
        printf("ACT_percLossInQoR %E\n", finalPercError);
        printf("ACT_percSumDifferent %E\n", (sumDifferent*100.0)/(float)(NX*NY*NZ));
        ...
        printf("ACT_percRelativeError_noskip %E\n", (sumRelativeError_noskip*100.0)/(float)(NX*NY*NZ));
