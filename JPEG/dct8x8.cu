...
float encode(byte *ImgSrc, byte *ImgDst, int Stride, ROI Size)
{
...
float decode(byte *ImgSrc, byte *ImgDst, int Stride, ROI Size)
{
...
int main(int argc, char** argv)
{
  ...
  int res = PreLoadBmp(SampleImageFname, &ImgWidth, &ImgHeight);
	...
	//load sample image
  LoadBmpAsGray(SampleImageFname, ImgStride, ImgSize, ImgSrc);
  ...
  float time;
  if(encoding) {
    time = encode(ImgSrc, ImgDstCUDA1, ImgStride, ImgSize);
  } else {
    time = decode(ImgSrc, ImgDstCUDA1, ImgStride, ImgSize);
  }
  printf("%f\n",time);
  //printf("Output filename %s\n", OutputImageFname);
	...
	// Finalization
	//
        DumpBmpAsGray("JPEGOUTPUT.bmp", ImgDstCUDA1, ImgStride, ImgSize);
	//release byte planes
	FreePlane(ImgSrc);
